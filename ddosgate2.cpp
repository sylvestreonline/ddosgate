...
const std::string LOCAL_IP = "votre_ip_locale"; // Remplacez par l'adresse IP de votre serveur
...
int main(int argc, char* argv[]) {
    ...
    TELEGRAM_TOKEN = "TELEGRAM_TOKEN"; // Remplacer par votre token Telegram
    TELEGRAM_CHAT_ID = "TELEGRAM_CHAT_ID"; // Remplacer par votre ID de chat Telegram
    ...
    pcap_t *handle = pcap_open_live("ens3", BUFSIZ, 1, 1000, errbuf);
    ...
    std::string filter_exp = "src not " + LOCAL_IP;
    struct bpf_program fp;

    if (pcap_compile(handle, &fp, filter_exp.c_str(), 0, PCAP_NETMASK_UNKNOWN) == -1) {
        std::cerr << "Impossible de compiler le filtre : " << pcap_geterr(handle) << std::endl;
        return 1;
    }

    if (pcap_setfilter(handle, &fp) == -1) {
        std::cerr << "Impossible de définir le filtre : " << pcap_geterr(handle) << std::endl;
        return 1;
    }
